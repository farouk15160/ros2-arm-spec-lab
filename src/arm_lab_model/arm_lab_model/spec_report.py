...
class SpecReport:
    ...
    def joint_table(self, payload: Optional[float] = None) -> List[Dict[str, Any]]:
        ...
        for i, joint in enumerate(cfg.joints):
            rows.append({
                ...
                'inertial_source': 'measured_assembly' if joint.link.inertial else 'tube_estimate',
                'com_link_m': joint.link.com_xyz.tolist(),
                'inertia_com_link_kgm2': joint.link.inertia_in_link_frame().tolist(),
    ...
    def as_dict(self) -> Dict[str, Any]:
        return {
            ...
            'assumptions': [
                'Fixed-base serial rotary arm; measured inertials replace complete link assembly estimates.',
                'Tube geometry still determines collisions and beam stiffness, including with measured inertials.',
                'Analytical tool inertia is a rigid box; moving jaw inertia/contact needs separate validation.',
                'Power includes ambient copper loss when electrical data exist; otherwise heuristic; no regeneration credit.',
                'Numerical agreement verifies equations and export, not hardware calibration or safety.',
            ],
